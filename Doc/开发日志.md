...
为避免过拟合，设计 4 个**通用性**实验：

#### 实验 A: Leading Role 过滤增强
**假设**: "partnership/connection/cooperation" 关键词表明合作关系，不是领导角色

**修改**: `argument_composer.py` 的 `_extract_organization_name()`
```python
# 新增过滤关键词
non_leadership_keywords = [
    "partner", "partnership", "connection", "facilitate",
    "cooperation", "collaborate", "work with"
]
```

#### 实验 B: 公司实体合并增强
**假设**: 同一法定代表人/创始人的公司应视为同一实体

**修改**: `entity_analyzer.py` 或 `argument_composer.py`
- 检测 "legal representative" 相同的公司
- 合并 "Co., Ltd." 和其品牌名

#### 实验 C: 媒体集团合并增强
**假设**: 同一媒体集团的不同版本/语言应合并

**修改**: `argument_composer.py` 的媒体分组逻辑
```python
# 媒体集团映射
media_group_merges = {
    "The Paper": "Sixth Tone",  # 同一集团中英文版
    "Oriental Morning Post": "Sixth Tone"
}
```

#### 实验 D: 完整性阈值过滤
**假设**: 完整性 < 40% 的论点证据不足，不应作为独立论点

**修改**: `argument_composer.py` 的 `compose_all()`
```python
# 过滤低完整性论点
MIN_COMPLETENESS = 40
filtered_args = [a for a in args if a.completeness["score"] >= MIN_COMPLETENESS]
```
