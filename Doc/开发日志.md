...
| 文件 | 操作 | 说明 |
|------|------|------|
| `services/entity_analyzer.py` | 新增 | LLM 分析实体，生成配置 |
| `services/argument_composer.py` | 重构 | 移除硬编码，使用配置 |
| `routers/arguments.py` | 修改 | 调用分析后再组合 |
...
{
  "applicant": {
    "formal_name": "Ms. Yaruo Qu",
    "name_variants": ["yaruo qu", "gaby", "ms. qu"]
  },
  "exhibit_mappings": {
    "media": {"D1": "The Jakarta Post", "D5": "China Sports Daily"},
    "associations": {"C1": "Shanghai Fitness Bodybuilding Association"},
    "organizations": {"F1": "Shanghai Yiqing Fitness Management Co., Ltd."}
  },
  "entity_merges": [
    {"canonical": "Shanghai Yiqing", "variants": ["venus weightlifting"]}
  ],
  "key_achievements": {
    "original_contribution": "BAT Training System",
    "awards": ["China Fitness Industry Achievement Award"]
  }
}
```
